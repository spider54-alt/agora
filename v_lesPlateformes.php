	...
	<section class="panel">
		...
		<div class="panel-body">
			...
			<tr>
				...
				<form action="index.php?uc=gererPlateformes" method="post">
				<td><?php echo $genre->identifiant; ?><input type="hidden"  name="txtIdPlateforme" value="<?php echo $plateforme->identifiant; ?>" /></td>
				<td><?php 
					if ($plateforme->identifiant != $idPlateformeModif) {
						...
						?>
							...
							<button class="btn btn-danger btn-xs" type="submit" name="cmdAction" value="supprimerPlateforme" title="Supprimer" onclick="return confirm('Voulez-vous vraiment supprimer cete plateforme?');"><i class="fa fa-trash-o "></i></button>
						</td>
					<?php
					}
					else {
